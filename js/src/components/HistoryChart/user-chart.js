...
const drawChart = () => {
  ...
  const addCircles = data => {
    ...
    selection
      ...
      .on("mouseover", function(d, i) {
        pointMouseOver(this, d, i);
      })
      .on("mouseout", function(d, i) {
        pointMouseOut(this, d, i);
      });
  ...
  const updateChart = () => {
    ...
    const newData = data.filter(d => d[3] <= endTime && d[3] >= startTime);
    ...
    throttle(event => {
      window.requestAnimationFrame(() => {
        ...
        let startTime = parseInt(
          moment(document.getElementById("start-time").value).valueOf()
        );
        let endTime = parseInt(
          moment(document.getElementById("end-time").value).valueOf()
        );
  ...
  const initializeTimeWindow = data => {
    ...
    const fromHash = hash
      .substr(1)
      .split(",")
      .map(e => new Date(parseInt(e)));
    return fromHash.map(d => {
      const datePrefix = `${d.getFullYear()}-${addZero(
        d.getMonth() + 1
      )}-${addZero(d.getDate())}`;
  ...
  let data = window.gon.data.map(d => [
    d.percentage,
    d.charging,
    d.updated_at,
    moment(d.updated_at).valueOf()
  ]);
  ...
  const xScale = d3
    .scaleTime()
    .domain([
      moment(initialFirstTime).valueOf(),
      moment(initialLastTime).valueOf()
    ])
    .range([0, graphWidth]);
